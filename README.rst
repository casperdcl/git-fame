...
This doesn't currently work with the alias (``git fame``), but does work with ``git-fame``.
...
.. code:: sh
    ...
    $ git-fame --print-completion bash |
      sudo tee /etc/bash_completion.d/git-fame_completion.bash

See `shtab <https://tqdm.github.io/shtab/#scripts>`_ for more information on where to place the completion script for your shell.
...
.. code:: sh
    ...
    git fame              # Alternative git alias (see above)
